...
```text
技术成长/Java/03-集合框架/04-HashMap.md
技术成长/数据与中间件/MySQL/03-事务隔离与MVCC.md
```

**过渡说明**：仓库中仍保留 `技术成长/Java/笔记/phase2-JVM/` 等历史复习手册目录。这些内容可继续阅读，但新增与重构时应迁入对应领域目录（如 JVM 迁入 `Java/08-JVM/` 或独立专题文件），不得在新 phase 目录下继续堆文档。

学习路线作为导航文档存在，例如 `技术成长/Java/7年Java工程师学习路线.md`，但**学习路线不得决定知识文件的物理位置**。
...
学习/
├── README.md                    # 学习总库入口
├── Repository-Specification.md  # 规范（副本，与 规范文档/ 同步）
├── Repository-Structure.md      # 目录蓝图（副本）
├── 规范文档/                    # 规范正文、迁移说明、模板（规划）
├── 技术成长/                    # ★ 正式技术知识主体
...
```text
技术成长/
├── README.md           # 技术成长总导航（必读）
├── 00-通用/            # 进度、复盘、求职、Cursor、工程素养、路线
├── Java/               # 语言、JVM、Spring、并发、面试专题
├── 数据与中间件/        # MySQL、Redis、消息队列、Elasticsearch
├── 计算机基础/          # 算法、计网、操作系统、安全
├── AI工程/             # Prompt、RAG、Agent、MCP 等
├── C++嵌入式/           # C++ 与嵌入式专项
└── 扩展语言/            # Go、Python 等（规划）
```
...
| 目录 | 职责 |
|------|------|
| `00-通用/` | 学习方法、进度追踪、复盘模板、求职追踪、项目实战清单；**迷路先看** [阅读指南.md](技术成长/00-通用/阅读指南.md) |
| `Java/` | Java 语言、集合、IO；JVM/Spring/并发等复习手册（逐步迁入领域子目录）；Java 面试与简历 |
| `数据与中间件/` | 关系库、缓存、MQ、搜索引擎等跨语言后端能力 |
| `计算机基础/` | 算法、网络、OS、安全 |
| `AI工程/` | LLM 应用开发与工程化 |
| `C++嵌入式/` | C++ 语言、嵌入式、岗位专项 |
| `扩展语言/` | 非主业语言的规划与笔记 |
...
规范文档不参与日常阅读路径。修改规范时须同步更新根目录同名副本（若保留）。
...
1. 读取 `规范文档/Repository-Specification.md`（或根目录副本）。
...
- 2026-07-01：从旧 `技术成长/Java/笔记/phase1-集合/` 迁移集合框架内容。
```
...
| 内容类型 | 位置 | 说明 |
|----------|------|------|
| MySQL / Redis / MQ / ES 深度专题 | `技术成长/数据与中间件/` | 已落地，保持 `00-速查` + `01~0N` 分章 |
| 算法 / 计网 / OS / 安全 | `技术成长/计算机基础/` | 已落地 |
| AI 实战 01～12 | `技术成长/AI工程/` | 已落地 |
| Java 集合专题 | `技术成长/Java/笔记/phase1-集合/` 或迁入 `Java/03-集合框架/` | 逐步按一文一题拆分 |
| JVM / Spring / 并发等复习手册 | `技术成长/Java/笔记/phase2~8/` | 过渡目录；重构时迁入 `Java/` 下领域子目录 |
| 进度 / 复盘 / 求职 | `技术成长/00-通用/` | 与面试简历、Java 面试文档配合 |
| Java 面试 / 简历 / 项目手册 | `技术成长/Java/` | 求职主线 |
| C++ 技术 | `技术成长/C++嵌入式/` | 与 Java 并行方向 |
