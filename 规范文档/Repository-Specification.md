...
长期知识库不得使用 `phase1`、`phase2`、`阶段一`、`入门阶段` 作为正式目录名称。学习阶段适合课程计划，不适合百科型知识库。正式目录应表达知识所属领域，例如 `技术成长/Java/笔记/phase1-集合/03-HashMap.md`，而不是 `Java/笔记/phase1-集合/HashMap.md`。

学习路线可以作为导航文档存在，例如 `技术成长/Java/7年Java工程师学习路线.md`，但学习路线不得决定知识文件的物理位置。
...
- `技术成长/Java/`：Java 语言本身，包括基础语法、面向对象、集合、泛型、异常、注解、反射、IO、新特性。
...
- `技术成长/数据与中间件/Redis/`：Redis 数据结构、持久化、高可用、缓存设计、分布式锁和生产调优。
- `技术成长/数据与中间件/消息队列/`：Kafka、RabbitMQ、RocketMQ、消息可靠性、顺序消息、事务消息、选型。
...
- `技术成长/AI工程/`：Prompt、LLM API、RAG、Embedding、Agent、MCP、评测、部署、安全、成本。
- `技术成长/C++嵌入式/`：C++ 语言、内存、STL、并发、嵌入式、从 Java 到 C++ 的迁移。
- `20-Project-Practice/`：企业级项目实战、业务场景、架构演进、故障复盘、案例库。
- `技术成长/00-通用/`：面试体系、题库、答题结构、简历、项目讲解、模拟面试。
- `技术成长/00-通用/`：个人成长、学习方法、技术管理、职业规划、复盘模板。
...
- Java phase 笔记：拆分迁移到 `技术成长/Java/`、`08-JVM/`、`09-Concurrency/`、`02-Spring-Ecosystem/`、`03-Database/`、`10-Distributed-Systems/`、`11-Architecture/`、`15-DevOps/`。
- 数据与中间件：迁移到 `03-Database/`、`技术成长/数据与中间件/Redis/`、`技术成长/数据与中间件/消息队列/`、`06-Middleware/`。
- 计算机基础：迁移到 `16-Computer-Science/`。
- AI 工程：迁移到 `技术成长/AI工程/`。
- C++ 嵌入式：迁移到 `技术成长/C++嵌入式/`，求职资料迁移到 `技术成长/00-通用/` 或 `技术成长/00-通用/`。
- Vue Demo：保留到 `前端/vue/`，正式前端知识重新规划到 `17-Frontend/`。
- Olympus EP7：保留到 `奥林巴斯ep7/EP7-Guide/`。
- 旧学习路线和复盘：迁移到 `技术成长/00-通用/` 或 `99-Archive/`。
